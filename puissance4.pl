...
getLineReview(B,NB_Line, Line,Index,Total_line):-nth0(Index, B, COL), nth0(NB_Line, COL, Element),append(Line,[Element],New_line), Index1 is Index+1,getLine(B,NB_Line, New_line,Index1,Total_line).


% Get the number of aligned pieces if we play at the provided column
get_nb_aligned_piecesReview(B,NB_COL,Player,INDEX_LIBRE, Nb_pieces_aligned):-count_vertical_piecesReview(B,Player,NB_COL,INDEX_LIBRE,1, Nb_pieces_vertical),count_horizontal_piecesReview(B,Player,NB_COL, INDEX_LIBRE,Nb_pieces_horizontal),count_diagonal_piecesReview(Player,NB_COL,INDEX_LIBRE,Nb_pieces_diagonal),max(Nb_pieces_vertical,Nb_pieces_horizontal,Max_vert_hor),max(Max_vert_hor,Nb_pieces_diagonal,Nb_pieces_aligned).
...
% Count the number of similar pieces that are under the last piece that we have put in the column (vertical count)
	...
	count_vertical_piecesReview(B,Player,NB_COL,INDEX_LIBRE,Last_count, Real_Count):- nth0(NB_COL, B, COL),Next_piece is INDEX_LIBRE+1, nth0(Next_piece,COL,Player), New_Count is Last_count+1,count_vertical_piecesReview(Player,NB_COL,Next_piece,New_Count, Real_Count).
...
% count the number of horizontal aligned pieces when we put a piece in the selected column
% NB_Line correspond to the INDEX_Libre (index in the column where we have put the piece)
	count_horizontal_piecesReview(B,Player,NB_COL, NB_Line,Count):- count_left_horizontal_piecesReview(Player,NB_COL,NB_Line,1, Count_left),count_right_horizontal_piecesReview(Player,NB_COL,NB_Line,0, Count_right), Count is Count_left+Count_right.
...
% INDEX_LIBRE is the index where we have put the piece
% Get the max of the right-aligned-diagonal, and the left-aligned-diagonal
count_diagonal_piecesReview(B,Player,NB_COL,INDEX_LIBRE,Count):- count_right_diagonal_pieces(B,Player,NB_COL,INDEX_LIBRE,Right_count), count_left_diagonal_piecesReview(B,Player,NB_COL,INDEX_LIBRE,Left_count), max(Right_count,Left_count,Count).

	count_right_diagonal_pieces(B,Player,NB_COL,INDEX_LIBRE,Count):- count_bottom_left_diagonal_pieces(B,Player,NB_COL,INDEX_LIBRE,1, Left_Count),count_top_right_diagonal_piecesReview(B,Player,NB_COL,INDEX_LIBRE,0, Right_Count), Count is Left_Count+Right_Count.
		% Count the number of piece on the right diagonal under the last put piece
	...
	count_left_diagonal_piecesReview(B,Player,NB_COL,INDEX_LIBRE,Count):- count_bottom_right_diagonal_pieces(Player,NB_COL,INDEX_LIBRE,1, Right_Count),count_top_left_diagonal_piecesReview(Player,NB_COL,INDEX_LIBRE,0, Left_Count), Count is Left_Count+Right_Count.
		% Count the number of piece on the left diagonal under the last put piece
