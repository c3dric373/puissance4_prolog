...
minmax(Board, Depth, MaximizingPlayer, Eval_score):-Depth==0,\+MaximizingPlayer,get_score_heur3(Board,'X',Res).
minmax(Board, Depth, MaximizingPlayer, Eval_score):-MaximizingPlayer,MaxEval is -10000,get_valid_moves(Board,L,Valid_moves), minmax_childs(Board, Depth, Valid_moves, MaximizingPlayer, Max_Eval, Res_Max_Eval), Eval_Score is Res_Max_Eval. 

minmax(Board, Depth, MaximizingPlayer, Eval_score):-\+MaximizingPlayer,MinEval is -10000,    get_valid_moves(Board,L,Valid_moves), minmax_childs(Board, Depth, Valid_moves,               MaximizingPlayer, Min_Eval, Res_Min_Eval), Eval_Score is Res_Min_Eval.
 


minmax_childs(Board, Depth, [], MaximizingPlayer, Max_Eval, Max_Eval):-!.
minmax_childs(Board, Depth,  [H|T], MaximizingPlayer, Max_Eval, Res_Max_Eval):-MaximizingPlayer, simulate_move(Board,H,'O', _,Res_Board),N_Depth is Depth -1, minmax(Res_Board,N_Depth, MaximizingPlayer, Eval_score),max(Eval_score, Max_Eval, New_Max_Eval), minmax_childs(Board, Depth, [T], MaximizingPlayer,New_Max_Eval, Res_Max_Eval). 

minmax_childs(Board, Depth,  [H|T], MaximizingPlayer, Min_Eval, Res_Min_Eval):-\+MaximizingPlayer, simulate_move(Board,H,'X', _,Res_Board),N_Depth is Depth -1,               minmax(Res_Board,N_Depth, MaximizingPlayer, Eval_score),min(Eval_score, Min_Eval, New_Min_Eval), minmax_childs(Board, Depth, [T], MaximizingPlayer,New_Min_Eval, Res_Min_Eval).
